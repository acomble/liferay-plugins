...
request.setAttribute(WebKeys.DOCUMENT_LIBRARY_FOLDER, null);
%>
...
<liferay-ui:icon-menu cssClass="select-existing-selector" icon='<%= themeDisplay.getPathThemeImages() + "/common/search.png" %>' id='<%= randomNamespace + "inputAssetLinks" %>' message="select" showWhenSingleIcon="<%= true %>">
	...
	for (AssetRendererFactory assetRendererFactory : AssetRendererFactoryRegistryUtil.getAssetRendererFactories(company.getCompanyId())) {
		String typeSelection = assetRendererFactory.getClassName();
		if (assetRendererFactory.isLinkable() && assetRendererFactory.isSelectable() && "com.liferay.portlet.documentlibrary.model.DLFileEntry".equals(typeSelection)) {
			if (assetEntryId > 0) {
			...
			assetBrowserURL.setParameter("typeSelection", typeSelection);
