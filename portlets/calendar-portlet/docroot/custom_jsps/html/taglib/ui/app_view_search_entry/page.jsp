...
<div class="app-view-entry app-view-search-entry-taglib entry-display-style <%= showCheckbox ? "selectable" : StringPool.BLANK %> <%= cssClass %>" data-title="<%= HtmlUtil.escapeAttribute(StringUtil.shorten(title, 60)) %>">
	<a class="entry-link fLeft width100" href="<%= url %>" title="<%= HtmlUtil.escapeAttribute(title + " - " + description) %>">
		<c:if test="<%= Validator.isNotNull(thumbnailSrc) %>">
			<div class="entry-thumbnail" style="min-height: 138px;">
		...
		<div class="entry-metadata fLeft" style="width:80%;">
			<span class="entry-title fLeft width100">
				<%= StringUtil.highlight(HtmlUtil.escape(title), queryTerms) %>
			</span>

			<c:if test="<%= Validator.isNotNull(description) %>">
			<span class="entry-description fLeft width100">
